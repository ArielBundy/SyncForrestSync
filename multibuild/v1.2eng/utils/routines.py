...
def displayMovie(filename, isFirst, checkFunc):
    ...
    if isFirst:
        left_bar_text = visual.TextStim(win=win, text="Sheep", pos=(-400, -450))
        right_bar_text = visual.TextStim(win=win, text="Wolf", pos=(400, -450))
    else:
        right_bar_text = visual.TextStim(win=win, text="Speech", pos=(400, -450))
...
def specify_experiment_phase(demographics_data):
    ...
    root = Tk()
    root.title("Social Synchrony Exp v1.2ENG")
...
def demographics():
    ...
    log_gui = gui.Dlg(title="personal info")
